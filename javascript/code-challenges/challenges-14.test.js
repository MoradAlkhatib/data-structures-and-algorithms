...
const screenForNames = (arr) => {
  // Solution code here...
 let reg=/^(Mr||Dr||Mrs||Ms).\s[A-Z]/
  let newArray=arr.filter(elem=>reg.test(elem) )
  return newArray
}
...
const toTitleCase = (arr) => {
  // Solution code here...
  let reg=/^\w/
  let newArray= arr.map(elem => elem.replace(reg,elem[0].toUpperCase()))
  return newArray
};
...
let biggerThanLuke = (arr) => {
  ...
  const str = arr.reduce((a, b) => {
    if (parseInt(b.mass) > parseInt(arr[0].mass)) {
      if (a) {
        a += ' - ' + b.name;
      } else {
        a = b.name;
...
const detectTicTacToeWin = (board) => {
  ...
  if (arr[0] == arr[1] && arr[0] == arr[2] && arr[0] != '') {
    ...
    return true;
  } else if (arr[2] == arr[4] && arr[2] == arr[6] && arr[0] != '') {
    return true;
  } else {
    return false;
  }
