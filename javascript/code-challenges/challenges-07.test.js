...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
   function reSort (a,b){return b.height-a.height;} 
   
  starWarsArr.sort(reSort)
  return starWarsArr;
}
...
const removeThree = (idx, arr) => {
  // Solution code here...
 let s= arr.filter((elem,index)=> {
  let r=!(index===idx || index===idx+1 ||index===idx+2)
...
const listFoods = (recipe) => {
  ...
  for(let i=0;i<arr.length;i++){
   ...
   let str= arr[i].slice(s+1)
    let a=str.indexOf(" ");
   result.push(str.slice(a+1)) ;
  }
