...
function transformToLis(obj){
  ...
  const {name ,age}=obj;
  if(obj.name){
    newArray.push(`<li>name: ${name}</li>`)
    newArray.push(`<li>age: ${age}</li>`)
    return newArray;
  }
...
Write a function named count that, given an integer and an array of arrays,
 uses either filter, map, or reduce to count the amount of times the integer is
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArray= input.map(elem=>
    elem.filter(item=> (item%5===0 && typeof(item)==="number")  )
     
   )

   let result=newArray.map(elem=>elem.map(item=> Math.pow(2,item)))
    console.log(result);
    return result;
...
let findShortest = (data) => {
  ...
  let test = data[0] 
    data.filter(item =>
       { if (parseInt(item.height) < parseInt(test.height))
           test = item}) 
             return test.name;
};
/* ------------------------------------------------------------------------------------------------
