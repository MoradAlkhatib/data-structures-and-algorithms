...
const checkValues = (obj, value) => {
 ...
 let onValue=values.map(elem => value===elem ?true:false)
 return onValue[0];
};
...
const hasChildrenValues = (arr, character) => {
 ...
 for (let j = 0; j < arr.length; j++) {
    let a = Object.values(arr[j]);
    let b = a[2];
    if (b) {
      if (typeof (b) == 'object') {
        for (let i = 0; i < b.length; i++) {
          if (b[i] === character) { bol = true; break; } 
          
        }
       if(bol){ break;}
      }
      else { if (b[j] === character) { bol = true; break; }else{ bol = false;} }
