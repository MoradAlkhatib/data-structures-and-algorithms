...
const longestString = (arr) => {
  ...
  let index= arr.findIndex(function(params) {
     return params == longest
...
const firstLetters = (arr) => {
  // Solution code here...
 let newArray= arr.map(item=> item[0])
 return newArray;
};
...
const findHappiness = (arr) => {
  ...
  let newArray=[];
  arr.forEach(item=> {
    if(reg.test(item)) newArray.push(item)})
    return newArray;
};
...
Write a function named standardizePhoneNumbers that takes in an array
 of phone number strings in (XXX) XXX-XXXX format and returns an array with the phone
  number strings in XXXXXXXXXX format.
...
const standardizePhoneNumbers = (arr) => {
  ...
  let newArray= arr.map(item=>
      item.split('').filter(elem=> reg.test(elem)).reduce(function (a,b) {
        return `${a}${b}`
...
const onlyOddChars = (str) => {
  // Solution code here...
  function funString(a,b) {
    return `${a}${b}`
    }
  let newString=str.split('').filter((item,index)=> index%2==1 ).reduce(funString,'')
...
const allHappy = (arr) => {
  ...
  let newBool= arr.map(item=> reg.test(item))
   let bool= newBool.reduce(checkBool,'true')
