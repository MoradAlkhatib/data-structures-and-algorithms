...
const replaceZeros = (string) => {
  // Solution code here...
 function reSort (a,b){return `${a}${b}`} 
 let reg = /0/; 
 let arrr=string.split('');
 console.log(arrr)
 let arr=arrr.map(elem=>{
  return reg.test(elem)  ?elem='zero':elem
 }).reduce(reSort)       
...
Write a function named validateWord that uses a regular expression
 pattern to validate that a word is between 5 and 10 characters long.
...
let result=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{3})+$/.test(email) ;
