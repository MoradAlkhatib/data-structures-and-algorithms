...
const toLastNames = people => {
  // Solution code here...
  let newArray=people.map(elem=> `${elem.firstName} ${elem.lastName}`)
  return newArray;
};
...
Write a function named addPurchases that, given an array of objects as input,
 uses reduce to find the total amount purchased. Each object contains the keys `item` 
 and `purchasePrice` like the example.
...
const addPurchases = (arr) => {
  // Solution code here...
  function add (a, b) {return a + b.purchasePrice};
  return arr.reduce(add , 0);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  function addIndex(total, num) {
    return arr.indexOf(num)+1;
  }

  let newArray=arr.reduce(addIndex,0)
  return newArray;
...
const returnNames = (arr) => {
  // Solution code here...
  function addName(total,obj) {
    newArray.push(obj.name)
     return obj.name;
  }

  let newArray=[]
  arr.reduce(addName,0)
  return newArray
};
